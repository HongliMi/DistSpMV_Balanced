...
int main(int argc, char ** argv)
{
    ...
    for (int r = 0; r < NTIMES; r++)
    {
...

     {
        int sum_mesg=0;
        for(int i=0;i<p;i++) {
            printf("process %d receive process %d communication is %d\n",id,i,length[i]);
              writeresults("./csv/DistSpMV_Reordered_com.csv", filename, id,i,length[i]);
            sum_mesg+=length[i];
        }
        ...
        writeresults2("./csv/DistSpMV_Reordered_calculation.csv", filename, id, sum_mesg, sum_local, sum_remote);
    }
    ...
    if (id == 0)
    {
        ...
        //writeresults("DistSpMV_Reordered.csv", filename, sourcematrix.rownum, sourcematrix.colnum, sourcematrix.nnznum,comtime,distime, distime+comtime, GFlops, p, nthreads);
    }
