...
void writeresults2(char *filename_res, char *filename,int id,int sum_mesg,int sum_local,int sum_remote);
void writeresults(char *filename_res, char *filename, int receiver,int sender,int num);
...
int main(int argc, char **argv)
{
    ...
    {   
        ...
        if (id == 0) {
            ...
            if(sdrc_time>cur_time) sdrc_time=cur_time;
        }
    }
    //mhl
  {
        int sum_mesg=0;
        for(int i=0;i<p;i++) {
            printf("process %d receive process %d calculation is %d\n",id,i,recv_cnt[i]);
            writeresults("./csv/DistSpMV_Balanced_com.csv", filename,id,i,recv_cnt[i]);
            sum_mesg+=recv_cnt[i];
        }
        printf("process %d receive total communication  is %d , local computation is %d , romote computation is %d\n",id,sum_mesg,sub_local_mtx.nnznum,sub_remote_mtx.nnznum);
        writeresults2("./csv/DistSpMV_Balanced_calculation.csv", filename,id,sum_mesg,sub_local_mtx.nnznum,sub_remote_mtx.nnznum);
    }
...
//         }
        ...
        if (id == 0) {
            gettimeofday(&t2, NULL);
            double cur_time=gettime(t1,t2);
            if(local_time>cur_time) local_time=cur_time;
        }
    }
...
//         }
        ...
        if (id == 0) {
            gettimeofday(&t2, NULL);
            double cur_time=gettime(t1,t2);
            if(remote_time>cur_time) remote_time=cur_time;
        }
    }
    /*
...
void writeresults(char *filename_res, char *filename, int receiver,int sender,int num)
{
    ...
    fprintf(fres, "%s, %d, %d, %d\n", filename, receiver, sender, num);
    fclose(fres);
}
...
void writeresults2(char *filename_res, char *filename,int id,int sum_mesg,int sum_local,int sum_remote)
{
    FILE *fres = fopen(filename_res, "a");
    if (fres == NULL) printf("Writing results fails.\n");
    fprintf(fres, "%s, %d, %d, %d, %d\n", filename, id, sum_mesg,sum_local, sum_remote);
    fclose(fres);
}
