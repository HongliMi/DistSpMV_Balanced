...
void writeresults(char *filename_res, char *filename, int m, int n, int nnzR, double pre_time);
...
//         }
    ...
    if (id == 0)
    {
        ...
        writeresults("./csv/DistSpMV_Balanced_preprocess.csv", filename, sourcematrix.rownum, sourcematrix.colnum, sourcematrix.nnznum,pre_time);
...
void writeresults(char *filename_res, char *filename, int m, int n, int nnzR, double pre_time)
{
    ...
    fprintf(fres, "%s, %d, %d, %d, %lf\n", filename, m, n, nnzR, pre_time);
    fclose(fres);
}