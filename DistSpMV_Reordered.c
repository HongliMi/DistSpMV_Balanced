...
int main(int argc, char ** argv)
{
    ...
    for (int r = 0; r < NTIMES; r++)
    {
        ...
        {
            gettimeofday(&t2, NULL);
            double ttime=((t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0);
            if(comtime>ttime) comtime=ttime;
...
*/
    ...
    for (int q = 0; q < NTIMES; q++)
    {
        ...
        if (id == 0)
        {
            gettimeofday(&t2, NULL);
            double ttime=((t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0);
            if(ttime<distime) distime=ttime;
    ...
    if (id == 0)
    {
        ...
        writeresults("./csv/DistSpMV_Reordered.csv", filename, sourcematrix.rownum, sourcematrix.colnum, sourcematrix.nnznum,comtime,distime, distime+comtime, GFlops, p, nthreads);
    }
